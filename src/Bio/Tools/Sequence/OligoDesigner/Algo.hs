...
designOligsDNA :: OligsSplittingConfig                -- ^ Splitting configuration
               -> [DNA]                               -- ^ DNA-sequence
               -> Except OligoDesignerError OligSet   -- ^ Result is set of oligs or error
designOligsDNA conf dna =
    case split conf (length dna) of
        Just splitting -> buildOligSet splitting dna
        _              -> throwError CannotFindSplitting
...
designOligsAA :: StdGen                            -- ^ Random generator
              ...
              -> [AA]                              -- ^ AA-sequence
              -> Except OligoDesignerError OligSet -- ^ Result is set of oligs with the best score or error
designOligsAA gen conf@(OligsDesignerConfig codonConf splittingConf _ _) aa = do
  ...
  where
    runOptimization :: [DNA] -> OligSplitting -> StateT StdGen (Except OligoDesignerError) OligSet
