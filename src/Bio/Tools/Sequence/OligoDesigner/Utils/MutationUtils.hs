...
import Bio.Tools.Sequence.OligoDesigner.Types             (Codon, Weight, OligoDesignerError(..))
...
mutate :: Organism -> [DNA] -> (Int, Int) -> StateT StdGen (Except OligoDesignerError) [[DNA]]
mutate organism dna interval@(start, end)
  | validateInterval interval (length dna) = throwError (InvalidInterval interval)
  | otherwise = do
...
mutateSlice :: Organism -> [DNA] -> StateT StdGen (Except OligoDesignerError) [[DNA]]
mutateSlice organism mutated = mutateEachCodon 0 [mutated]
  where
    mutateEachCodon :: Int -> [[DNA]] -> StateT StdGen (Except OligoDesignerError) [[DNA]]
...
oneMutation :: Organism -> Codon -> StateT StdGen (Except OligoDesignerError) Codon
oneMutation organism codon =
    case Map.lookup codon codon2ak of
        Nothing -> throwError (CannotFindCodon $ prettyDNA codon)
        Just aa -> do
...
randomCodon :: Organism -> AA -> StateT StdGen (Except OligoDesignerError) Codon
...
weightedRandom :: [(a, Weight)] -> StateT StdGen (Except OligoDesignerError) a
weightedRandom []    = throwError CannotGetRandom
weightedRandom items = do
  ...
  where
    getWeightedItem :: Double -> Double -> [(a, Weight)] -> Except OligoDesignerError a
    getWeightedItem _ _ []       = throwError OtherError
